                ...
                <form action="" method="post">
                    ...
                    <select required class="form-control mb-3" name="fk_garbage" id="sampah">
                        ...
                        <?php foreach ($listSampah as $sampah) : ?>
                            <option value="<?= $sampah->_id ?>" <?= $sampah->_id == $pemasukan->fk_garbage ? "selected" : "" ?>><?= $sampah->j_name ?></option>
                        <?php endforeach; ?>
                    </select>

                    <?= form_error('pm_jumlah', '<small class="text-danger pl-3">', '</small>') ?>
                    <label for="">Jumlah</label>
                    <input required type="number" class="form-control mb-3" name="pm_jumlah" min="1" value="<?= $pemasukan->pm_jumlah ?>" id="jumlah">

                    <?= form_error('pm_total', '<small class="text-danger pl-3">', '</small>') ?>
                    <label for="">Total</label>
                    <input required type="number" class="form-control mb-3" name="pm_total" min="1" value="<?= $pemasukan->pm_total ?>" id="total">
                    ...
                    <select required type="date" class="form-control mb-3" name="fk_pelapak">
                        <?php foreach ($listPelapak as $pelapak) : ?>
                            <option value="<?= $pelapak->_id ?>" <?= ($pemasukan->fk_pelapak == $pelapak->_id) ? "selected" : "" ?>><?= $pelapak->p_nama ?></option>
                        <?php endforeach; ?>
                    </select>

                    <?= form_error('pm_created_at', '<small class="text-danger pl-3">', '</small>') ?>
                    <label for="">Tanggal Penjualan</label>
                    <input required type="date" class="form-control mb-3" name="pm_created_at" value="<?= $pemasukan->pm_created_at ?>" />

                    <?= form_error('pm_hasil', '<small class="text-danger pl-3">', '</small>') ?>
                    <label for="">Keterangan</label>
                    <textarea required class="form-control mb-3" name="pm_hasil"><?= $pemasukan->pm_hasil ?></textarea>
                    <button type="submit" class="btn btn-sm btn-block btn-warning text-black">Ubah Pemasukan</button>
                </form>
            </div>
        </div>
    </div>
...
<script>
    const sampah = document.getElementById("sampah");
    const jumlah = document.getElementById("jumlah");
    const total = document.getElementById("total");
    let hargaSampah = 0;
    fetch(`<?= base_url() ?>pemasukan/get_harga_by_id/${sampah.value}`)
        .then(res => res.json())
        .then((result) => {
            hargaSampah = result.harga_pelapak;
            total.value = hargaSampah * jumlah.value;
        });
    sampah.addEventListener('change', (e) => {
        if (e.target.value !== "") {
