...
class Induk extends CI_Controller
{
    ...
    function index()
    {
        ...
        wrapper_templates($this, "dashboard/induk", $data);
    }
    ...
    function update($id)
    {
        if ($id !== "") {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($update) {
                    _set_flashdata($this, 'message', 'success', 'Update Request Sampah Berhasil', 'induk');
                } else {
                    _set_flashdata($this, 'message', 'danger', 'Update Request Sampah Gagal', 'induk');
                }
            }
        } else {
            _set_flashdata($this, 'message', 'danger', 'ID Invalid', 'induk');
        }
    }
            ...
            if ($id !== "") {
                ...
                if ($thing) {
                    ...
                    if ($upload["status"]) {
                        ...
                        if ($update) {
                            _set_flashdata($this, 'message', 'success', 'Ubah Foto Request Sampah Berhasil', 'induk');
                        } else {
                            _set_flashdata($this, 'message', 'danger', 'Ubah Foto Request Sampah Gagal', 'induk');
                        }
                    } else {
                        ...
                        _set_flashdata($this, 'message', 'danger', $ind[$eng], 'induk');
                    }
                } else {
                    _set_flashdata($this, 'message', 'danger', 'Harap masukkan ID yang valid', 'induk');
                }
            } else {
                _set_flashdata($this, 'sampah', 'danger', 'Harap Masukkan ID terlebih dahulu', 'induk');
            }
        } else {
            _set_flashdata($this, 'sampah', 'danger', 'Harap isi gambar terlebih dahulu', 'induk');
        }
    }
    ...
    function accept_request($id)
    {
        if ($id) {
            ...
            if ($accept) {
                _set_flashdata($this, 'message', 'success', 'Anda Berhasil mengonfirmasi request', 'induk');
            } else {
                _set_flashdata($this, 'message', 'danger', 'Anda Gagal mengonfirmasi request', 'induk');
            }
        } else {
            _set_flashdata($this, 'message', 'danger', 'Silahkan Masukkan ID yang valid', 'induk');
        }
    }
    ...
    function cancel_request($id)
    {
        if ($id) {
            ...
            $accept = $this->RequestSampah_model->update_request($update, $where);
            if ($accept) {
                _set_flashdata($this, 'message', 'success', 'Anda Berhasil membatalkan request', 'induk');
            } else {
                _set_flashdata($this, 'message', 'danger', 'Anda Gagal membatalkan request', 'induk');
            }
        } else {
            _set_flashdata($this, 'message', 'danger', 'Silahkan Masukkan ID yang valid', 'induk');
        }
    ...
    function create_transaction($req)
    {
        ...
        if ($transaksi) {
            ...
            _set_flashdata($this, 'message', 'danger', 'Anda Gagal membuat transaksi', 'induk');
        }
    }
    ...
    function create_mutation($req)
    {
        ...
        if ($mutation) {
            ...
            _set_flashdata($this, 'message', 'danger', 'Anda Gagal membuat mutasi', 'induk');
        }
    }
    ...
    function add_balance($amount, $fk_nasabah)
    {
        ...
        if ($add) {
            _set_flashdata($this, 'message', 'success', 'Request telah selesai', 'induk');
        } else {
            _set_flashdata($this, 'message', 'danger', 'Anda Gagal menambah balance ke nasabah', 'induk');
        }
    }
